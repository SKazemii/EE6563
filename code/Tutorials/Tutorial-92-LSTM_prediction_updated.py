...
# load the dataset
dataframe = read_csv(
    "/Users/saeedkazemi/Documents/Python/EE6563/Dataset/Ass2/Processed_NYSE.csv",
    usecols=[1],
    engine="python",
)
...
model1 = load_model(
    "C:/Users/pkumar1/OneDrive - University of New Brunswick/time_series_course/Tutorial/totorial-1/LSTM_weights.best.hdf5"
)
# make predictions
...
plt.bar(
    ["RMSE (Basic approach)", "RMSE (Save & Load)", "RMSE (Time Step)"],
    p_data,
    color="b",
    width=0.25,
)
...
# reading the dataset
series = pd.read_csv(
    "C:/Users/pkumar1/OneDrive - University of New Brunswick/time_series_course/Tutorial/totorial-1/AirPassengers.csv"
)
...
# X_lg_1diff  =difference(X_lg,1)
#
##3. Lets take 12th difference
# X_lg_12diff  =difference(X_lg_1diff,12)
...
# 4. AR model
train_size = int(len(X_lg) * 0.67)
test_size = len(X_lg) - train_size
X_lg = np.expand_dims(X_lg, 1)
train, test = X_lg[0:train_size, :], X_lg[train_size : len(X_lg), :]
...
# inverted12 = [inverse_difference(X_lg_1diff[i], X_lg_12diff[i]) for i in range(len(X_lg_12diff))]
# inverted1 = [inverse_difference(X_lg[i], inverted12[i]) for i in range(len(inverted12))]
#
orig_test = np.exp(test)
transform_pred = np.exp(predictions)
ar_rmse = sqrt(mean_squared_error(orig_test, transform_pred))
print("Test RMSE: %.3f" % ar_rmse)
...
plt.bar(
    ["RMSE (Basic approach)", "RMSE (Save & Load)", "RMSE (Time Step)", "ar_rmse"],
    p_data,
    color="b",
    width=0.25,
)
